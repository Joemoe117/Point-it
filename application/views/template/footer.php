
<!-- load les scripts écrits dans des fichiers a part d'une façon propre pas comme au dessus -->
<script type="text/javascript" src="<?= base_url('/assets/js/get_points.js') ?>"></script>
<script type="text/javascript" src="<?= base_url('/assets/js/form_valid_add.js') ?>"></script>
<script type="text/javascript" src="<?= base_url('/assets/js/perso/approuve.js') ?>"></script>

<script type="text/javascript">
	<script>
	$(function() {
		$( document ).tooltip();
	});
</script>
</script>

<div class="footer">
	<div class="container">
		<div class="row">
			<div class="col-md-4">
				<div class="footer_titre"> Point-it </div>
				Point-it est le fils spirituel de Pointbook, un projet plutôt stylé qui a démarré en 2012.
			</div>
			<div class="col-md-4">
				<div class="footer_titre"> Crédits </div>
				Ce site a été codé par <b>Baptiste</b> et <b>Thomas</b>, deux développeurs alcooliques de qualité! <br>
				<p>Merci à Gwenael pour son aide au design et Youenn pour les logo des points ! Mort aux social-traitres Solène !</p>
			</div>
			<div class="col-md-4">
				<div class="footer_titre"> Technologies utilisées </div>
				<img src="<?= base_url("/assets/images/footer/codeigniter.png");  ?>" alt="CodeIgniter, framework de qualité" class="footer-image">
				<img src="<?= base_url("/assets/images/footer/jQuery.gif");  ?>" alt="Jquery, parce que sinon le JS c'est chiant" class="footer-image">
				<img src="<?= base_url("/assets/images/footer/bootstrap.png");  ?>" alt="Boostrap, cause i'm not a webdesigner" class="footer-image">
				<img src="<?= base_url("/assets/images/footer/github.png");  ?>" alt="Github, Parce que plus on de fous, plus on rit" class="footer-image">
			</div>
		</div>
	</div>
</div>
</body>
</html>
